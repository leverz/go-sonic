...
type Searchable interface {
	...
	Query(collection, bucket, terms string, limit, offset int, lang string) (results []string, err error)
...
func (s searchChannel) Query(collection, bucket, term string, limit, offset int, lang string) (results []string, err error) {
	if lang == "" {
		lang = "none"
	}
	err = s.write(fmt.Sprintf("%s %s %s \"%s\" LIMIT(%d) OFFSET(%d) LANG(%s)", query, collection, bucket, term, limit, offset, lang))
	if err != nil {
		return nil, err
	}
